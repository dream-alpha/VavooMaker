...
def applySkinFactor(*d):
    ...
    return tuple([int(value * getSkinFactor())
                 if isinstance(value, (int, float)) else value for value in d])


def SelectionEntryComponent(
    description,
    value,
    index,
    selected,
    selectionListDescr=parameters.get(
        "SelectionListDescr",
        applySkinFactor(
            25,
            0,
    ...
    if selected:
        selectionpng = LoadPixmap(
            cached=True, path=resolveFilename(
                SCOPE_CURRENT_SKIN, "icons/lock_on.png"))
    else:
        selectionpng = LoadPixmap(
            cached=True, path=resolveFilename(
                SCOPE_CURRENT_SKIN, "icons/lock_off.png"))
    ix, iy, iw, ih = parameters.get(
        "SelectionListLock", applySkinFactor(
            0, 2, 25, 24))
    res.append(
...
class SelectionList(MenuList):
    def __init__(self, list=None, enableWrapAround=False):
        MenuList.__init__(self, list or [], enableWrapAround,
                          content=eListboxPythonMultiContent)
        font = fonts.get("SelectionList", applySkinFactor("Regular", 20, 30))
        self.l.setFont(0, gFont(font[0], font[1]))
        self.l.setItemHeight(font[2])
        self.selectionListDescr = parameters.get(
            "SelectionListDescr", applySkinFactor(25, 0, 650, 30))
    ...
    def toggleSelection(self):
        if len(self.list):
            ...
            self.list[idx] = SelectionEntryComponent(
                item[0], item[1], item[2], not item[3], self.selectionListDescr)
            self.setList(self.list)

    def getSelectionsList(self):
        return [(item[0][0], item[0][1], item[0][2])
                for item in self.list if item[0][3]]
    ...
    def applySkin(self, desktop, parent):
        ...
        for x in range(len(self.list)):
            ...
            self.list[x] = SelectionEntryComponent(
                description, value, index, selected, self.selectionListDescr)
        self.setList(self.list)

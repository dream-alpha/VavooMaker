...
class vavooFetcher():
    ...
    def getPlaylist(self):
        current = self.playlists_processed.get(
            config.plugins.vavoomaker.current.value, {})
        if not current:
            self.downloadPage()

        known_urls = []
        json_data = os_path.join(self.tempDir,
                                 config.plugins.vavoomaker.current.value)
    ...
    def createBouquet(self, enabled):
        ...
        for country in sorted([k for k in current.keys(
        ) if k in enabled], key=lambda x: group_titles.get(x, x).lower()):
            """
            # bouquet_list = []
            ...
            if current[country]:  # if the country is not empty
                ...
                for channelname, url in sorted(current[country]):
                    url = url.strip() + str(app)
                    bouquet_list.append(
                        "#SERVICE 4097:0:1:1:1:1:CCCC0000:0:0:0:%s:%s" %
                        (url.replace(
                            ":", "%3a"), channelname))
            if bouquet_list:
                bouquet_filename = sanitizeFilename(
                    country).replace(" ", "_").strip().lower()
    ...
    def removeBouquetReference(self, bouquet_filename):
        ...
        if os_path.exists(bouquets_file):
            try:
                ...
                print(
                    "[vavoo plugin] Bouquet entry removed from bouquets.tv:",
                    bouquet_filename)
            except Exception as e:
    ...
    def removeAllVavooBouquets(self):
            ...
            if file.startswith("userbouquet.vavoo") and file.endswith(".tv"):
                ...
                if os_path.exists(bouquet_path):
                    ...
                    try:
                        ...
                        print(
                            "[vavoo plugin] Error removing bouquet:", file, e)
                else:
        ...
        if os_path.exists(bouquets_file) and removed_bouquets:
            try:
                ...
                with open(bouquets_file, "w") as f:
                    for line in lines:
                        if not any(bouquet.lower() in line.lower()
                                   for bouquet in removed_bouquets):
                            f.write(line)
...
class SetupMaker(Screen):
    ...
    def __init__(self, session):
        ...
        self.title = _("vavoo playlists") + " - " + choices.get(
            config.plugins.vavoomaker.current.value,
            config.plugins.vavoomaker.current.value).title()
    ...
    def buildList(self):
        ...
        self.process_build = sorted(list(self.vavooFetcher.playlists_processed[config.plugins.vavoomaker.current.value].keys(
        )), key=lambda x: group_titles.get(x, x).lower())
        self.enabled = [
            x for x in getattr(
                config.plugins.vavoomaker,
                config.plugins.vavoomaker.current.value).value.split("|") if x in self.process_build]
        self["config"].setList([SelectionEntryComponent(group_titles.get(
            x, x), x, "", x in self.enabled) for x in self.process_build])
    ...
    def readList(self):
        ...
        getattr(
            config.plugins.vavoomaker,
            config.plugins.vavoomaker.current.value).value = "|".join(
            self.enabled)
            ...
            if answer:
                ...
                if self.enabled:
                    ...
                    self["description"].text = _(
                        "Creating bouquets. This may take some time. Please be patient.")
                    self["key_red"].text = ""
                    self["key_green"].text = ""
                    ...
                    if hasattr(self.runtimer, "callback"):
                        ...
                        if os_path.exists("/usr/bin/apt-get"):
                            self.runtimer_conn = self.runtimer.timeout.connect(
                                self.doRun)
                        print(
                            "[Version Check] ERROR: eTimer does not support callback.append()")
                    self.runtimer.start(10, 1)
                else:
                    self.session.open(
                        MessageBox,
                        _("Please select the bouquets you wish to create."),
    ...
    def backCancel(self):
        self.readList()
        if any([getattr(config.plugins.vavoomaker, choice).isChanged()
    ...
    def deleteBouquets(self):
        ...
        def onConfirm(answer):
            if answer:
                ...
                self.session.open(
                    MessageBox,
                    _("Reloading Bouquets and Services...\n\nAll Vavoo Favorite Bouquets removed."),
                    MessageBox.TYPE_INFO,
                    timeout=5)
            else:
                self.session.open(
...
def Plugins(**kwargs):
    return [
        PluginDescriptor(
            ...
            description=_("Make IPTV bouquets based on Vavoo List Channels"),

...
TMPPATH=/tmp/VavooMaker-main
FILEPATH=/tmp/main.tar.gz

# Determine plugin path based on architecture
if [ ! -d /usr/lib64 ]; then
    PLUGINPATH=/usr/lib/enigma2/python/Plugins/Extensions/vavoo-maker
else
    PLUGINPATH=/usr/lib64/enigma2/python/Plugins/Extensions/vavoo-maker
fi
...
echo "Downloading VavooMaker..."
wget --no-check-certificate 'https://github.com/Belfagor2005/VavooMaker/archive/refs/heads/main.tar.gz' -O "$FILEPATH"
if [ $? -ne 0 ]; then
    echo "Failed to download VavooMaker package!"
    exit 1
fi

tar -xzf "$FILEPATH"
if [ $? -ne 0 ]; then
    echo "Failed to extract VavooMaker package!"
    exit 1
fi

cp -r 'VavooMaker-main/usr' '/'
set +e
